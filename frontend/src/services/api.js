...
function requestWithTimeout(url, options = {}, timeoutMs = REQUEST_TIMEOUT_MS) {
...
function normalizeNetworkError(error, fallbackMessage) {
  if (error?.name === 'AbortError') {
    return createError('The backend took too long to respond. Please retry.', 408);
  }

  return createError(error?.message || fallbackMessage, error?.status || 500);
}
...
export async function transcribeAudio({ audio, language }) {
  if (!audio || audio.size === 0) {
    throw createError('No audio was captured. Please record again.');
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw createError(
        await readErrorMessage(response, 'Transcription failed. Please retry.'),
        response.status,
      );
    }
    ...
    throw normalizeNetworkError(error, 'Transcription failed. Please retry.');
  }
}

export async function extractProfileFromTranscript({ transcript, language }) {
  if (!transcript?.trim()) {
    throw createError('The transcript was empty, so a profile could not be extracted.');
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw createError(
        await readErrorMessage(response, 'Profile extraction failed. Please retry.'),
        response.status,
      );
    }
    ...
    throw normalizeNetworkError(error, 'Profile extraction failed. Please retry.');
  }
}
